...
\begin{enumerate}
\item Importar el proyecto en Eclipse  BLA
\item Disponer de una conexión a la base de datos \emph{Oracle 10g} en el puerto 11521 de \verb#localhost#. 
\item Ejecutar GenPadron pasándole como argumentos \verb#../resources/votantes/roles.txt# y \verb#../resources/votacion/votaciones.txt#. Esto genera el padrón de votación con los archivos de entrada.
\item Ejecutar el servidor de MesaApp (en el caso de interpretar a la mesa).
\item Ejecutar el servidor de UrnaApp (en el caso de interpretar a la urna).
\item Ejecutar el cliente de VotanteApp (en el caso de interpretar al votante).
\end{enumerate}

Para realizar luego el conteo se debe ejecutar Contador pasándole como argumento el ID de la votación que se desea contabilizar.
...
\subsubsection{Características no implementadas}
Contador no verifica que los comprobantes de la mesa hayan sido generados por los votantes.
