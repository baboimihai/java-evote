...
El votante genera un sobre encriptando con las claves de los partidos la boleta elegida concatenada a un valor aleatorio $\text{RandB}_i$ para evitar que la mesa pueda regenerar todos los posibles sobres y saber a quién votó la persona.
...
Despúes de almacenar el voto, la urna informa a la mesa que la persona ya votó (para evitar que vuelva a votar) dándole $U_U \left( S_{{VU}_i} \right)$ que es el valor que la mesa utilizó para relacionar al votante. También le envia un \emph{Challenge} para que la mesa le devuelva firmado a modo de reconocimiento (TODO: Expresar mejor esto) . La mesa guarda por ese votante el par de datos $U_{V_i}$, $R_{V_i} \Bigl[ \text{DNI} + \text{ID}_V + \text{Hash} \bigl( U_U \left( S_{{VU}_i} \right) + S_{{VM}_i}\bigr) \Bigr]$ a modo de comprobante de voto.

\subsubsection{Paso 6}
\begin{footnotesize}
\begin{equation}
\text{Mesa} \rightarrow \text{Urna}: R_U \bigl( R_M \left( \text{Challenge} \right) \bigr) 
\end{equation}
\end{footnotesize}
...
El conteo debe ser realizado en una maquina en la cual confien todos los partidos. La urna debe presentar los sobres y la mesa la lista con comprobantes de voto. Para abrir los sobres cada partido tiene que ingresar su clave privada. El conteo se realiza utilizando las cadenas $R_M \left(\text{Opc}_i + \text{ID}_V + \text{RandA}_i \right)$, que como fueron firmadas por la mesa garantizan la validez del sobre. 
...
Informalmente: Si se terminó el proceso de votación, la urna tendría más $R_M \bigl( U_U \left( S_{{VU}_i} \right) \bigr)$ que comprobantes de voto.

\subsubsection{Generar votos inválidos}
No puede generar los comprobantes del tipo $U_{V_i}$, $R_{V_i} \Bigl[ \text{DNI}+ \text{ID}_V + \text{Hash} \Bigl( U_U \left( S_{{VU}_i} \right) + S_{{VM}_i}\Bigr) \Bigr]$ porque utilizan la clave privada de cada votante.
...
La urna no puede generar un sobre válido, porque la opción tiene que estar firmado por la mesa. Si decide modificar un sobre aleatoriamente lo estaría impugnando, pero como el usuario tiene como ticket un \textbf{hash} de ese sobre sabría que su voto fue modificado y podría denunciarlo.

\subsubsection{Mover un voto válido de una elección a otra}
No puede porque las boletas firmadas por la mesa contienen el ID de la votación.

\subsubsection{Agregar o sacar votos}
La urna tiene que almacenar los $R_M \bigl( U_U \left( S_{{VU}_i} \right) \bigr)$ recibidos en el paso 2, cuya cantidad debe ser la misma que la cantidad de sobres. Caso cotrario, hubo fraude de parte de la urna. Si saca un sobre, el votante puede darse cuenta por lo explicado en la subsección \ref{modif_voto}. Si agrega un sobre, no va a poder generar un secreto $S_{VU}$ encriptado firmado.
